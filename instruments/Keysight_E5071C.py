...
class Keysight_E5071C(Instrument):
	'''
...
#		'''
#		resp = self._visainstrument.query('CALC'+str(1)+':PAR:CAT?').strip('"')
#		names = resp.split(',')
...
#		self._visainstrument.write("DISP:WIND:TRAC:Y:AUTO")
	...
	def avg_status(self):
		self._visainstrument.write('SENS:AVER?')
	...
	def get_tracedata(self, format = 'AmpPha'):
		...
		try:
			...
			#data = self._visainstrument.ask_for_values(':FORMAT REAL32; FORMat:BORDer SWAP;*CLS; CALC:DATA? SDATA;*OPC',format=visa.single) 
			#data = self._visainstrument.query(':FORMAT REAL32;CALC:DATA? SDATA;',format=visa.double) 
			#data = self._visainstrument.query('FORM:DATA REAL; FORM:BORD SWAPPED; CALC%i:SEL:DATA:SDAT?'%(self._ci), format = visa.double)	  
			#test
			data = self._visainstrument.query_binary_values("CALCulate:DATA:SDAT?",datatype=u'f') 
		finally:
...
#		return self._zerospan
	...
	def do_set_trigger_source(self,source):
		...
		Input:
			source (string) : INTernal | EXTernal | MANual | BUS)

		Output:
			None
		'''
		logging.debug(__name__ + ' : setting trigger source to "%s"' % source)
		if source.upper() in ["INT", "EXT", "MAN", "BUS"]:
			self._visainstrument.write(':TRIG:SOUR %s' % source.upper())		
		else:
			raise ValueError('set_trigger_source(): must be INT | EXT | MANual | BUS')
